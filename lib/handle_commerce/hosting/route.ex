defmodule HandleCommerce.Hosting.Route do
  use Ecto.Schema
  ...
  Parse a route string into the pattern matching segments
      ...
      iex> HandleCommerce.Hosting.Route.parse("/")
      {:ok, %HandleCommerce.Hosting.Route{segments: []}}

  Segments can be deeply nested too.

      iex> HandleCommerce.Hosting.Route.parse("/a/b/c")
      {:ok, %HandleCommerce.Hosting.Route{segments: [segment: "a", segment: "b", segment: "c"]}}

  Matching parameters is done by a leading colon ":"

      iex> HandleCommerce.Hosting.Route.parse("/products/:id/details")
      {:ok,
  ...
  Glob matching can be done to capture the rest of the URL if necessary
      ...
      iex> HandleCommerce.Hosting.Route.parse("/blog/*id")
      {:ok, %HandleCommerce.Hosting.Route{segments: [segment: "blog", glob: "id"]}}

  However, the glob match must come a the end of a parsed route, otherwise
  you will receive an error.

      iex> HandleCommerce.Hosting.Route.parse("/blog/*id/remaining")
